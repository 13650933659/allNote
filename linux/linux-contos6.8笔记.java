...
2、安装linux
	...
	2、安装centOS6.8(安装之前进入pc的bios->security->Intel..Technology->开启,要不然安装不了)
		...
		2、虚拟机硬件的配置
			...
			3、网络配置
				1、默认是NAT：不会ip冲突，linux可以通过主机服务和主机同一网的机器，但是反之不行(cent7用的是这一种)
	...
	3、安装centos7时
		1、注意要选择桌面(兼容win)
		2、可以参考 https://www.cnblogs.com/lishiwei/p/10823206.html （他包括了 NAT 的链接）
3、使用
	1、终端的使用：鼠标右键即可看到
	...
	7、vi和vim编辑器，vim是vi的增强版
		...
		2、快捷键(其他的去看文档)
			...
			14、gVim是Windows里的vi
		3、多行注释 参考： https://blog.csdn.net/step_ma/article/details/76474864
			1、第一种
				1）首先按键盘上的ESC进入命令行模式
				2）再在底行输入以下命令
					:起始行号,结束行号s/^/注释符/g
		...
		2、linux下的ip获取方案
			...
			2、静态ip(推荐使用这一种)
				...
				2、最少要改下面5句
					...
					IPADDR=192.168.216.7
					GATEWAY=192.168.216.1
					DNS1=114.114.114.114
					NETMASK=255.255.255.0    #新增：上面步骤1和3的图中的子网掩码
				3、service network restart：重启网络服务
				4、ping www.baidu.com ping 主机(192.168.216.12) 说明可以和外网和主机通信了
...
　　	ls -lR|grep "^d"|wc -l
	...
	7、 删除超多文件
		find /data/pdfGenerator/pdfout -name "*.pdf" |xargs rm -r
