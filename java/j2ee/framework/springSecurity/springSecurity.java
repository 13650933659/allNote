...
5、
	1、 认证服务器
		TokenEndpoint.postAccessToken(Principal principal, @RequestParamMap<String, String> parameters) 
			ClientDetails authenticatedClient = getClientDetailsService().loadClientByClientId(clientId);			// 获取第三方应用的信息（有可能也是我们自己公司的应用，或者前端）
			TokenRequest tokenRequest = getOAuth2RequestFactory().createTokenRequest(parameters, authenticatedClient);	// 根据请求参数创建 TokenRequest
			oAuth2RequestValidator.validateScope(tokenRequest, authenticatedClient);			// 验证 scope 是否有效
			OAuth2AccessToken token = getTokenGranter().grant(tokenRequest.getGrantType(), tokenRequest);		// CompositeTokenGranter 实现的获取 OAuth2AccessToken(令牌)


http 请求头带了 Authorization : Basic 或者 bearer
